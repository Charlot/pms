...
module FileHandler
  module CSV
      ...
      begin
        ...
        if validate_msg.result
          Machine.transaction do
            resource_group = ResourceGroup.find_by_name(row['Resource Group'])
            ...
            if machine
              ...
              machine = Machine.new({nr:row['Nr'],name:row['Name'],description:row['Description'],resource_group_id:resource_group.id,id:row['IP']})
              machine.save
    ...
    def self.validate_row(row)
      ...
      unless ResourceGroup.find_by_name(row['Resource Group'])
        msg.contents << "Resource Group:#{row['Resource Group']}不存在"
      end

      unless Machine.find_by_id(row['Row']).nil?
